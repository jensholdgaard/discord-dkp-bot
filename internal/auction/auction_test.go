...
func TestPlaceBid(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setup: func() *auction.Auction {
				return auction.New("a1", "Sword of Truth", "admin", 10, 5*time.Minute)
			},
			playerID:  "p1",
			amount:    50,
			playerDKP: 100,
		...
		},
		{
			...
			setup: func() *auction.Auction {
				return auction.New("a2", "Shield", "admin", 100, 5*time.Minute)
			},
			playerID:  "p1",
			amount:    50,
			playerDKP: 200,
		...
		},
		{
			...
			setup: func() *auction.Auction {
				return auction.New("a3", "Helm", "admin", 10, 5*time.Minute)
			},
			playerID:  "p1",
			amount:    150,
			playerDKP: 100,
		...
		},
		{
			name: "self outbid",
			setup: func() *auction.Auction {
				a := auction.New("a4", "Boots", "admin", 10, 5*time.Minute)
		...
		},
		{
			...
			setup: func() *auction.Auction {
				a := auction.New("a5", "Ring", "admin", 10, 5*time.Minute)
		...
		},
		{
			...
			setup: func() *auction.Auction {
				a := auction.New("a6", "Cloak", "admin", 10, 5*time.Minute)
				_ = a.PlaceBid(context.Background(), "p1", 50, 100)
				return a
			},
			playerID:  "p2",
...
func TestAuction_Close(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setup: func() *auction.Auction {
				a := auction.New("a1", "Sword", "admin", 10, 5*time.Minute)
				_ = a.PlaceBid(context.Background(), "p1", 50, 100)
		...
		},
		{
			...
			setup: func() *auction.Auction {
				return auction.New("a2", "Shield", "admin", 10, 5*time.Minute)
			},
			wantWinner: false,
		},
		{
			...
			setup: func() *auction.Auction {
				a := auction.New("a3", "Helm", "admin", 10, 5*time.Minute)
				_, _ = a.Close(context.Background())
				return a
			},
			wantErr: auction.ErrAuctionClosed,
...
func TestAuction_ConcurrentBids(t *testing.T) {
	a := auction.New("concurrent-test", "Epic Item", "admin", 1, 5*time.Minute)
...
func TestAuction_Replay(t *testing.T) {
	...
	original := auction.New("replay-test", "Legendary Sword", "admin", 10, 5*time.Minute)
...
func TestAuction_PendingEvents(t *testing.T) {
	a := auction.New("events-test", "Item", "admin", 10, 5*time.Minute)
	_ = a.PlaceBid(context.Background(), "p1", 50, 100)
