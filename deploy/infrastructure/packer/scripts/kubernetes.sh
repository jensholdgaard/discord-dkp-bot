...
# Force IPv4 for apt: pkgs.k8s.io returns 403 over Hetzner IPv6
echo 'Acquire::ForceIPv4 "true";' > /etc/apt/apt.conf.d/99force-ipv4

mkdir -p /etc/apt/keyrings/
# Force IPv4 (-4): pkgs.k8s.io returns 403 when accessed from Hetzner via IPv6
curl -4 --silent --show-error --location --fail \
    https://pkgs.k8s.io/core:/stable:/v${TRIMMED_KUBERNETES_VERSION}/deb/Release.key \
